...
export default function IndustryInput({ value, onChange, placeholder = 'Industry type…', style }: Props) {
  ...
  const filtered = industries.filter(d => d.name.toLowerCase().includes(query.toLowerCase()))
  const exactMatch = industries.some(d => d.name.toLowerCase() === query.toLowerCase())
  ...
  async function addNew() {
    ...
    await createIndustry.mutateAsync(query.trim())
    select(query.trim())
  }
