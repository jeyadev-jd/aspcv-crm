...
import { useDiscussions, useCreateDiscussion, useDeleteDiscussion, DISCUSSION_TYPES, DISCUSSION_CATEGORIES } from '@/hooks/useDiscussions'
...
export default function DiscussionPanel({ entityType, entityId, contacts = [] }: Props) {
  ...
  return (
    <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div>
              ...
              <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} style={inpStyle}>
                {DISCUSSION_CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
              </select>
            </div>
            <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <button onClick={handleCreate} disabled={!form.title.trim() || createDiscussion.isPending}
                style={{ padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 600, border: 'none', background: '#5D78FF', color: '#fff', cursor: 'pointer', opacity: !form.title.trim() ? 0.5 : 1 }}>
      ...
      {discussions.length === 0 && !showForm && (
        <p style={{ fontSize: 11, color: '#B1B1BE', textAlign: 'center', padding: '16px 0' }}>No discussions yet. Click + Add to log one.</p>
      )}
