...
interface Props {
  ...
  isPending?: boolean
  /**
   * Seconds the confirm button stays disabled. Use on genuinely irreversible
   * actions so a double-click cannot blow past the dialog.
   */
...
export default function ConfirmDialog({
  ...
  danger = true, onConfirm, onCancel, isPending = false, countdownSeconds = 0,
}: Props) {
  ...
  // Escape cancels (unless a mid-action pending state is in flight).
  useEffect(() => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 70 }}>
      <div role="dialog" aria-modal="true" aria-label={title} style={{ background: '#fff', borderRadius: 16, padding: 24, width: 360, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <button
            ...
            disabled={disabled}
            style={{
              flex: 1, padding: 10, borderRadius: 10, fontSize: 12, fontWeight: 600, border: 'none',
              background: disabled ? '#D5D5D5' : danger ? '#FF5353' : '#5D78FF',
              color: '#fff', cursor: disabled ? 'not-allowed' : 'pointer',
              opacity: isPending ? 0.7 : 1,
            }}
          >
            {isPending ? 'Working…' : locked ? `${confirmLabel} (${remaining})` : confirmLabel}
          </button>
        </div>
      </div>
    </div>
