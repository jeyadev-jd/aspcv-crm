...
import { Paperclip, Upload, X, FileText, Image } from 'lucide-react'
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 10 }}>
          {uploads.map(a => (
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 6 }}>
              {a.mimeType.startsWith('image/') ? <Image size={14} color="#3b82f6" /> : <FileText size={14} color="#64748b" />}
              <a href={`http://localhost:4000${a.url}`} target="_blank" rel="noreferrer" style={{ fontSize: 13, color: '#374151', flex: 1, textDecoration: 'none' }}>{a.fileName}</a>
