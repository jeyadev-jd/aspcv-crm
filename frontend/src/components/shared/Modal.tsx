...
export default function Modal({ title, onClose, children, footer, maxWidth = 520, zIndex = 60 }: Props) {
  return (
    ...
    >
      <div style={{ background: '#fff', borderRadius: 16, width: `min(${maxWidth}px, 96vw)`, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid #F0F1F5', position: 'sticky', top: 0, background: '#fff', zIndex: 1 }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: '#374557' }}>{title}</p>
          <button onClick={onClose} style={{ color: '#B1B1BE', background: 'none', border: 'none', cursor: 'pointer', padding: 4, display: 'flex' }}>
            <X size={16} />
          </button>
        </div>
